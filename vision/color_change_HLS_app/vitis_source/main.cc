...
void exit_system(AXI_VDMA<ScuGicInterruptController>& vdma_driver, OV5640& cam, VideoOutput& vid)
{
	vdma_driver.resetWrite();
	vdma_driver.resetRead();
	vid.reset();
...
int main()
{
	...
	while (1) {
		...
		case 'q':
			xil_printf("\r\n  Are you sure wanted to quit? (y)");
			read_char1 = getchar();
			getchar();
			switch(read_char1) {
			case 'y':
				xil_printf("\r\n   Exiting application");
				exit_system(vdma_driver, cam, vid);
				exit(0);
			default:
				break;
			}
